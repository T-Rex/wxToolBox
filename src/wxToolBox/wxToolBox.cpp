...
WX_DEFINE_OBJARRAY(wxToolBoxIconArray)
...
void wxToolBox::RefreshTabs()
{
	...
	if(m_SelectedTab)
	{
		...
		{
			wxLogTrace(wxTraceMask(), _("Selected tab %s [%ix%i] child (%i,%i) - (%ix%i)"),				
		...
		if(tab1)
		{
			...
			if(bOK)
			{
				...
				if(tab2)
				{
					...
					wxLogTrace(wxTraceMask(), wxT("DownScroll Y = %i"), tab1->GetRectangle().GetTop());
					bScrollDnSet=true;
				}
...
void wxToolBox::OnSize(wxSizeEvent & event)
{
	...
	if(m_SelectedTab)
	{
		...
		wxLogTrace(wxTraceMask(), wxT("Selected tab rect [%i,%i], [w=%i,h=%i]"),
