...
: 	wxFrame(parent, -1, _("wxToolBox SkinEditor"), wxPoint(100,100), wxSize(900,680)),
m_PropGrid(NULL)
{	
	SetMinSize(GetSize());
...
void ToolBoxTestFrame::CreateToolBox(wxWindow * parent)
{
	...
	m_ToolBox = new wxToolBox(parent, 10001, wxDefaultPosition, wxSize(160,300), wxNO_BORDER);
...
{	
	...
	m_PropGrid->Append(wxEnumProperty(wxT("Style"), wxPG_LABEL, styles));
	m_PropGrid->Append(wxBoolProperty(wxT("ItemsTransparent"), wxPG_LABEL, m_ToolBox->GetItemsTransparent()));
	m_PropGrid->Append(wxBoolProperty(wxT("SelectedItemTransparent"), wxPG_LABEL, m_ToolBox->GetSelectedItemTransparent()));
	m_PropGrid->Append(wxBoolProperty(wxT("UseBackground"), wxPG_LABEL, m_ToolBox->GetUseBackground()));
	m_PropGrid->Append(wxBoolProperty(wxT("UseStdArrows"), wxPG_LABEL, m_ToolBox->GetUseStdArrows()));
	m_PropGrid->Append(wxBoolProperty(wxT("PreDrawBackgrounds"), wxPG_LABEL, m_ToolBox->GetPreDrawBackgrounds()));
	m_PropGrid->Append(wxBoolProperty(wxT("SelectNewTab"), wxPG_LABEL, m_ToolBox->GetSelectNewTab()));
	m_PropGrid->Append(wxBoolProperty(wxT("ShowSelectionAlways"), wxPG_LABEL, m_ToolBox->GetShowSelectionAlways()));
	m_PropGrid->Append(wxColourProperty(wxT("BackgroundColour"),wxPG_LABEL,
		m_ToolBox->GetBackgroundColour()));
	m_PropGrid->Append(wxColourProperty(wxT("BorderHighlightColour"),wxPG_LABEL,
		m_ToolBox->GetBorderHighlightColour()));
	m_PropGrid->Append(wxColourProperty(wxT("BorderShadowColour"),wxPG_LABEL,
		m_ToolBox->GetBorderShadowColour()));
	
	m_PropGrid->Append(wxAdvImageFileProperty(wxT("Background")));
	
	m_PropGrid->Append(wxPropertyCategory(wxT("Items"),wxPG_LABEL));	

	m_PropGrid->Append(wxToolObjectBackgroundInfoProperty(wxT("Item Background"),wxPG_LABEL));
	m_PropGrid->Append(wxFontProperty(wxT("ItemNormalFont"),wxPG_LABEL));
	m_PropGrid->Append(wxFontProperty(wxT("ItemHoverFont"),wxPG_LABEL));
	m_PropGrid->Append(wxFontProperty(wxT("ItemSelectedFont"),wxPG_LABEL));
	m_PropGrid->Append(wxColourProperty(wxT("ItemTextColour"),wxPG_LABEL,
		m_ToolBox->GetItemTextColour()));

	m_PropGrid->Append(wxIntProperty(wxT("ItemHeight"),wxPG_LABEL,m_ToolBox->GetItemHeight()));
	m_PropGrid->Append(wxIntProperty(wxT("ItemSpacing"),wxPG_LABEL,m_ToolBox->GetItemSpacing()));
	m_PropGrid->Append(wxIntProperty(wxT("ItemCaptionSpacing"),wxPG_LABEL,m_ToolBox->GetItemCaptionSpacing()));

	m_PropGrid->Append(wxColourProperty(wxT("ItemNormalColour"),wxPG_LABEL,
		m_ToolBox->GetItemNormalColour()));	
	m_PropGrid->Append(wxColourProperty(wxT("ItemHoverColour"),wxPG_LABEL,
		m_ToolBox->GetItemHoverColour()));	
	m_PropGrid->Append(wxColourProperty(wxT("ItemSelectedColour"),wxPG_LABEL,
		m_ToolBox->GetItemSelectedColour()));	

	m_PropGrid->Append(wxBoolProperty(wxT("UseItemNormalBackground"), wxPG_LABEL, m_ToolBox->GetUseItemNormalBackground()));
	m_PropGrid->Append(wxBoolProperty(wxT("UseItemHoverBackground"), wxPG_LABEL, m_ToolBox->GetUseItemHoverBackground()));
	m_PropGrid->Append(wxBoolProperty(wxT("UseItemSelectedBackground"), wxPG_LABEL, m_ToolBox->GetUseItemSelectedBackground()));

	m_PropGrid->Append(wxPropertyCategory(wxT("Tabs"),wxPG_LABEL));	

	m_PropGrid->Append(wxToolObjectBackgroundInfoProperty(wxT("Tab Background"),wxPG_LABEL));
	m_PropGrid->Append(wxFontProperty(wxT("TabNormalFont"),wxPG_LABEL));
	m_PropGrid->Append(wxFontProperty(wxT("TabHoverFont"),wxPG_LABEL));
	m_PropGrid->Append(wxFontProperty(wxT("TabSelectedFont"),wxPG_LABEL));
	m_PropGrid->Append(wxColourProperty(wxT("TabTextColour"),wxPG_LABEL,
		m_ToolBox->GetTabTextColour()));

	m_PropGrid->Append(wxIntProperty(wxT("TabHeight"),wxPG_LABEL,m_ToolBox->GetTabHeight()));
	m_PropGrid->Append(wxIntProperty(wxT("TabSpacing"),wxPG_LABEL,m_ToolBox->GetTabSpacing()));
	m_PropGrid->Append(wxIntProperty(wxT("TabCaptionSpacing"),wxPG_LABEL,m_ToolBox->GetTabCaptionSpacing()));

	m_PropGrid->Append(wxColourProperty(wxT("TabNormalColour"),wxPG_LABEL,
		m_ToolBox->GetTabNormalColour()));	
	m_PropGrid->Append(wxColourProperty(wxT("TabHoverColour"),wxPG_LABEL,
		m_ToolBox->GetTabHoverColour()));	
	m_PropGrid->Append(wxColourProperty(wxT("TabSelectedColour"),wxPG_LABEL,
		m_ToolBox->GetTabSelectedColour()));	

	m_PropGrid->Append(wxBoolProperty(wxT("UseTabBackground"), wxPG_LABEL, m_ToolBox->GetUseTabBackground()));

	m_PropGrid->Append(wxPropertyCategory(wxT("Buttons"),wxPG_LABEL));	

	m_PropGrid->Append(wxToolObjectBackgroundInfoProperty(wxT("Button Background"),wxPG_LABEL));
	m_PropGrid->Append(wxColourProperty(wxT("ButtonNormalColour"),wxPG_LABEL,
		m_ToolBox->GetButtonNormalColour()));	
	m_PropGrid->Append(wxColourProperty(wxT("ButtonHoverColour"),wxPG_LABEL,
		m_ToolBox->GetButtonHoverColour()));	
	m_PropGrid->Append(wxColourProperty(wxT("ButtonSelectedColour"),wxPG_LABEL,
		m_ToolBox->GetButtonSelectedColour()));	

	m_PropGrid->Append(wxBoolProperty(wxT("UseButtonBackground"), wxPG_LABEL, m_ToolBox->GetUseButtonBackground()));
}
...
void ToolBoxTestFrame::OnPropertyChanged(wxPropertyGridEvent & event)
{
	...
	if(name == wxT("Style"))
	{
		m_ToolBox->SetStyle(event.GetPropertyValueAsInt());
	}
	if(name == wxT("Background"))
	{
		m_ToolBox->SetBackground(event.GetPropertyValueAsString());
	}
	if(name == wxT("UseBackground"))
	{
		m_ToolBox->SetUseBackground(!!event.GetPropertyValueAsBool());
	}
	if(name == wxT("UseTabBackground"))
	{
		m_ToolBox->SetUseTabBackground(!!event.GetPropertyValueAsBool());
	}
	if(name == wxT("UseItemNormalBackground"))
	{
		m_ToolBox->SetUseItemNormalBackground(!!event.GetPropertyValueAsBool());
	}
	if(name == wxT("UseItemHoverBackground"))
	{
		m_ToolBox->SetUseItemHoverBackground(!!event.GetPropertyValueAsBool());
	}
	if(name == wxT("UseItemSelectedBackground"))
	{
		m_ToolBox->SetUseItemSelectedBackground(!!event.GetPropertyValueAsBool());
	}
	if(name == wxT("UseButtonBackground"))
	{
		m_ToolBox->SetUseButtonBackground(!!event.GetPropertyValueAsBool());
	}
	if(name == wxT("UseStdArrows"))
	{
		m_ToolBox->SetUseStdArrows(!!event.GetPropertyValueAsBool());
	}
	if(name == wxT("SelectNewTab"))
	{
		m_ToolBox->SetSelectNewTab(!!event.GetPropertyValueAsBool());
	}
	if(name == wxT("ShowSelectionAlways"))
	{
		m_ToolBox->SetShowSelectionAlways(!!event.GetPropertyValueAsBool());
	}
	if(name == wxT("PreDrawBackgrounds"))
	{
		m_ToolBox->SetPreDrawBackgrounds(!!event.GetPropertyValueAsBool());
	}
	if(name == wxT("ItemsTransparent"))
	{
		m_ToolBox->SetItemsTransparent(!!event.GetPropertyValueAsBool());
	}
	if(name == wxT("SelectedItemTransparent"))
	{
		m_ToolBox->SetSelectedItemTransparent(!!event.GetPropertyValueAsBool());
	}
	if(name == wxT("BackgroundColour"))
	{		
		m_ToolBox->SetBackgroundColour(*((wxColour*)event.GetPropertyValueAsWxObjectPtr()));
		m_ToolBox->Refresh();
	}
	if(name == wxT("BorderHighlightColour"))
	{		
		m_ToolBox->SetBorderHighlightColour(*((wxColour*)event.GetPropertyValueAsWxObjectPtr()));
		m_ToolBox->Refresh();
	}
	if(name == wxT("BorderShadowColour"))
	{		
		m_ToolBox->SetBorderShadowColour(*((wxColour*)event.GetPropertyValueAsWxObjectPtr()));
		m_ToolBox->Refresh();
	}

	if(name == wxT("TabNormalColour"))
	{		
		m_ToolBox->SetTabNormalColour(*((wxColour*)event.GetPropertyValueAsWxObjectPtr()));
	}
	if(name == wxT("TabHoverColour"))
	{		
		m_ToolBox->SetTabHoverColour(*((wxColour*)event.GetPropertyValueAsWxObjectPtr()));
	}
	if(name == wxT("TabSelectedColour"))
	{		
		m_ToolBox->SetTabSelectedColour(*((wxColour*)event.GetPropertyValueAsWxObjectPtr()));
	}
	if(name == wxT("TabNormalFont"))
	{
		m_ToolBox->SetTabNormalFont(*((wxFont*)event.GetPropertyValueAsWxObjectPtr()));
	}
	if(name == wxT("TabHoverFont"))
	{
		m_ToolBox->SetTabHoverFont(*((wxFont*)event.GetPropertyValueAsWxObjectPtr()));
	}
	if(name == wxT("TabSelectedFont"))
	{
		m_ToolBox->SetTabSelectedFont(*((wxFont*)event.GetPropertyValueAsWxObjectPtr()));
	}
	if(name == wxT("TabTextColour"))
	{		
		m_ToolBox->SetTabTextColour(*((wxColour*)event.GetPropertyValueAsWxObjectPtr()));
	}
	if(name == wxT("TabHeight"))
	{
		m_PropGrid->SetPropertyValue(name, abs(event.GetPropertyValueAsInt()));
		m_ToolBox->SetTabHeight(abs(event.GetPropertyValueAsInt()));
	}
	if(name == wxT("TabSpacing"))
	{
		m_PropGrid->SetPropertyValue(name, abs(event.GetPropertyValueAsInt()));
		m_ToolBox->SetTabSpacing(abs(event.GetPropertyValueAsInt()));
	}

	if(name == wxT("ItemNormalColour"))
	{
		m_ToolBox->SetItemNormalColour(*((wxColour*)event.GetPropertyValueAsWxObjectPtr()));
	}
	if(name == wxT("ItemHoverColour"))
	{		
		m_ToolBox->SetItemHoverColour(*((wxColour*)event.GetPropertyValueAsWxObjectPtr()));
	}
	if(name == wxT("ItemSelectedColour"))
	{		
		m_ToolBox->SetItemSelectedColour(*((wxColour*)event.GetPropertyValueAsWxObjectPtr()));
	}
	if(name == wxT("ItemNormalFont"))
	{
		m_ToolBox->SetItemNormalFont(*((wxFont*)event.GetPropertyValueAsWxObjectPtr()));
	}
	if(name == wxT("ItemHoverFont"))
	{
		m_ToolBox->SetItemHoverFont(*((wxFont*)event.GetPropertyValueAsWxObjectPtr()));
	}
	if(name == wxT("ItemSelectedFont"))
	{
		m_ToolBox->SetItemSelectedFont(*((wxFont*)event.GetPropertyValueAsWxObjectPtr()));
	}
	if(name == wxT("ItemTextColour"))
	{		
		m_ToolBox->SetItemTextColour(*((wxColour*)event.GetPropertyValueAsWxObjectPtr()));
	}
	if(name == wxT("ItemHeight"))
	{
		m_PropGrid->SetPropertyValue(name, abs(event.GetPropertyValueAsInt()));
		m_ToolBox->SetItemHeight(abs(event.GetPropertyValueAsInt()));
	}
	if(name == wxT("ItemSpacing"))
	{
		m_PropGrid->SetPropertyValue(name, abs(event.GetPropertyValueAsInt()));
		m_ToolBox->SetItemSpacing(abs(event.GetPropertyValueAsInt()));
	}

	if(name == wxT("ButtonNormalColour"))
	{		
		m_ToolBox->SetButtonNormalColour(*((wxColour*)event.GetPropertyValueAsWxObjectPtr()));
	}
	if(name == wxT("ButtonHoverColour"))
	{		
		m_ToolBox->SetButtonHoverColour(*((wxColour*)event.GetPropertyValueAsWxObjectPtr()));
	}
	if(name == wxT("ButtonSelectedColour"))
	{		
		m_ToolBox->SetButtonSelectedColour(*((wxColour*)event.GetPropertyValueAsWxObjectPtr()));
	}
...
bool ToolBoxTestFrame::GetPaneVisibility(wxString pane_name)
{
	...
	{		
		...
		{
			wxLogTrace(wxTraceMask(), wxT("%s: %i"), pane_name, (int)all_panes.Item(i).IsShown());
